...
## Messaging policy (updated 2026-05-12)

CLAIRE owns the canonical morning briefing. CODE-claw does NOT duplicate calendar, task lists, overdue surfacing, follow-ups, or deadline radar — those all live in the 07:30 CLAIRE briefing.

CODE-claw should ONLY send a Telegram message if one of these infrastructure conditions is true:
- Todoist API failure during this run (e.g., 401, rate-limit, network) — report the exact error
- Calendar / Apple Notes MCP failure that affected the prep
- NanoClaw service degradation observed during the run (Ollama, QMD, Honcho, container runtime)
- A genuinely novel safety condition this skill caught (e.g., a recurring task got duplicated 5+ times, the priority map file is missing, Todoist returned an empty list for the first time in >30 days)

Do NOT message Mike about:
- His calendar today (CLAIRE handles)
- Overdue tasks (CLAIRE handles)
- This-week deadlines (CLAIRE handles)
- Conflicts in the schedule (CLAIRE handles)
- "FYI" items, normal task surfacing, daily focus suggestions

If the run completes cleanly with no infra issues, stay silent.
