...
> **Status: SHIPPED 2026-04-04 (Layer 1 + Layer 2).** Layer 1 (in-process auto-fix): `src/health-monitor.ts` defines `FixHandler` (line 54), `addFixHandler` (105), `getFixHandler` (109), `getInfraFailureCount` (117), `attemptFix` (121), `createDefaultFixActions` (464). 6 of 7 fix scripts present in `scripts/fixes/` (restart-{qmd,apple-notes,todoist,container-runtime}.sh, kill-port-squatter.sh, kill-sqlite-orphans.sh — `restart-simplemem.sh` retired with SimpleMem→Honcho migration, expected). 155/155 health-monitor tests pass. Layer 2 (external watchdog): `scripts/watchdog-heartbeat.sh` (7.2K) + `launchd/com.nanoclaw.watchdog.plist` shipped. **Reloaded into launchd 2026-04-25** after audit found it had drifted out (`launchctl load ~/Library/LaunchAgents/com.nanoclaw.watchdog.plist`); first tick exit-code 0, health endpoint verified responsive at `http://127.0.0.1:3003/health`, no Layer 1 lock contention. Confirmed prior auto-recovery in log (2026-04-22 detected stale dist drift of 27hr, rebuilt + kicked launchctl). Open `- [ ]` checkboxes left as-is — banner is the source of truth.
