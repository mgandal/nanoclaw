...
> **Status: SHIPPED 2026-04-12 (mechanism); case study superseded.** Mechanism shipped: guard script `scripts/guards/inbox-has-unread.py` (3.4K) live; `src/task-scheduler.ts:153 runGuardScript` exported and called at line 265-266 against `task.script`; `scheduled_tasks.script` column in schema. 11/11 guard tests pass via `bun test tests/task-scheduler-guard.test.ts`. **Task 3 (the SQL UPDATE wiring `nanoclaw-inbox-monitor`) was intentionally not applied** — the inbox-monitor case study was superseded by the email-ingest pipeline (`scripts/sync/email-ingest.py` + `email_ingest/` package, plan `2026-04-11-passive-email-ingestion.md`). The script mechanism itself is in active production use by 5 tasks (3 active, 2 completed): `gmail-plus-monitor.py` (Gmail+ filter), `inbox-convert.py` (vault PDF/DOCX→MD), Reddit r/LocalLLaMA daily fetcher, plus 2 one-shots (rebuild-and-restart, CLAUDE.md patcher). All use the `{"wakeAgent": bool, "data": ...}` JSON contract from the plan. Reverified `nanoclaw-inbox-monitor` task ID does not exist in `scheduled_tasks` 2026-04-25. Open `- [ ]` checkboxes left as-is — banner is the source of truth.
