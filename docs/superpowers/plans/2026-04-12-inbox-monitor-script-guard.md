...
> **Status: PARTIALLY SHIPPED 2026-04-12 — ⚠ Task 3 DB wire-up never applied.** Tasks 1, 2, 4 shipped: guard script `scripts/guards/inbox-has-unread.py` (3.4K) live; `src/task-scheduler.ts:153 runGuardScript` exported and called at line 265-266 against `task.script`; `scheduled_tasks.script` column added to schema. 11/11 guard tests pass via `bun test tests/task-scheduler-guard.test.ts`. **Gap:** Task 3 (the SQL UPDATE wiring `nanoclaw-inbox-monitor` to use `script`) was never applied — no scheduled_tasks row currently has a `script` value, and there is no `nanoclaw-inbox-monitor` task ID registered. Net effect: guard infrastructure runs only when a task explicitly opts in via `script=`, which no task currently does. The targeted ~90% reduction in agent invocations is not in effect. Operator action: either run the missing UPDATE, or document that the inbox monitor was retired in favor of the email-ingest pipeline (which subsumes its function). Open `- [ ]` checkboxes left as-is — banner is the source of truth.
