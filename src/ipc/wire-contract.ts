 ...
 *
 * The canonical copy lives at container/agent-runner/src/wire-contract.ts
 * (inside the image build context). The host build cannot import across
 * that boundary, so the two copies are kept in lockstep by
 * src/ipc/wire-contract.test.ts, which deep-compares the exports and
 * pins every registered handler to this table. Edit BOTH together.
 */
