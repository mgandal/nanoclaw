...
export function insertPendingAction(input: PendingActionInput): string {
  ...
  // type, summary, and semantic payload) returns the existing row
  // instead of stacking duplicate /approve entries every time an agent
  // retries a staged call it mistook for a failure. Summary is part of
  // the identity so two genuinely different actions that happen to share
  // a payload shape stay distinct.
