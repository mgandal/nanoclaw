...
# assert_contains SUBSTRING [VARNAME]
# Fails the test if VARNAME (default: $output) does not contain SUBSTRING.
# bats-core 1.x does NOT abort on a failing `[[ ]]` alone — only the LAST
# command in a test determines pass/fail. Use this helper for mid-test asserts.
assert_contains() {
  local needle="$1"
  local haystack="${2:-$output}"
  if [[ "$haystack" != *"$needle"* ]]; then
    echo "assert_contains: expected to find '$needle' in output" >&2
...
# assert_not_contains SUBSTRING [VARNAME]
assert_not_contains() {
  local needle="$1"
  local haystack="${2:-$output}"
  if [[ "$haystack" == *"$needle"* ]]; then
    echo "assert_not_contains: expected NOT to find '$needle' in output" >&2
    echo "--- actual output ---" >&2
    echo "$haystack" >&2
    echo "--- end ---" >&2
    return 1
